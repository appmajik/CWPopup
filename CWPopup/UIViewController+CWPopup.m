...
// This method is taken from Apple's UIImageEffects category provided in WWDC
// 2013 sample code
- (UIImage *)applyBlurWithRadius:(CGFloat)blurRadius
                       tintColor:(UIColor *)tintColor
           saturationDeltaFactor:(CGFloat)saturationDeltaFactor
    ...
    if (self.size.width < 1 || self.size.height < 1) {
        NSLog(@"*** error: invalid size: (%.2f x %.2f). Both dimensions must "
              @"be >= 1: %@",
    ...
    if (maskImage && !maskImage.CGImage) {
        NSLog(@"*** error: maskImage must be backed by a CGImage: %@",
              maskImage);
        return nil;
    }
    ...
    BOOL hasSaturationChange =
        fabs(saturationDeltaFactor - 1.) > __FLT_EPSILON__;
    if (hasBlur || hasSaturationChange) {
        ...
        if (hasBlur) {
            // A description of how to compute the box kernel width from the
            // Gaussian
            ...
            // used: Three
            // successive box-blurs build a piece-wise quadratic convolution
            // kernel, which
            ...
            NSUInteger radius =
                floor(inputRadius * 3. * sqrt(2 * M_PI) / 4 + 0.5);
            if (radius % 2 != 1) {
                radius += 1;  // force radius to be odd so that the three
                              // box-blur methodology works.
            }

            vImageBoxConvolve_ARGB8888(&effectInBuffer, &effectOutBuffer, NULL,
                                       0, 0, (uint32_t)radius, (uint32_t)radius,
                                       0, kvImageEdgeExtend);
            vImageBoxConvolve_ARGB8888(&effectOutBuffer, &effectInBuffer, NULL,
                                       0, 0, (uint32_t)radius, (uint32_t)radius,
                                       0, kvImageEdgeExtend);
            vImageBoxConvolve_ARGB8888(&effectInBuffer, &effectOutBuffer, NULL,
                                       0, 0, (uint32_t)radius, (uint32_t)radius,
        ...
        if (hasSaturationChange) {
            ...
            CGFloat floatingPointSaturationMatrix[] = {
                ...
                0, 0.7152 - 0.7152 * s, 0.7152 + 0.2848 * s,
                0.7152 - 0.7152 * s, 0, 0.2126 - 0.2126 * s,
                0.2126 - 0.2126 * s, 0.2126 + 0.7873 * s, 0, 0, 0, 0, 1,
            };
...
- (UIImage *)getBlurredImage:(UIImage *)imageToBlur {
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0) {
        return [imageToBlur applyBlurWithRadius:10.0f
...
- (void)addBlurView {
    ...
    objc_setAssociatedObject(self, &CWBlurViewKey, blurView,
...
- (void)presentPopupViewController:(UIViewController *)viewControllerToPresent
    ...
    if (self.popupViewController == nil) {
        ...
        CGRect finalFrame =
            [self getPopupFrameForViewController:viewControllerToPresent];
// parallax setup if iOS7+
...
#endif
        // shadow setup
        viewControllerToPresent.view.layer.shadowOffset =
            CGSizeMake(0.0f, 0.0f);
        viewControllerToPresent.view.layer.shadowColor =
            [UIColor blackColor].CGColor;
        viewControllerToPresent.view.layer.shadowRadius = 3.0f;
        viewControllerToPresent.view.layer.shadowOpacity = 0.8f;
        viewControllerToPresent.view.layer.shadowPath =
            [UIBezierPath bezierPathWithRect:viewControllerToPresent.view.layer
                                                 .bounds].CGPath;
        // rounded corners
        ...
        if (self.useBlurForPopup) {
            ...
            objc_setAssociatedObject(self, &CWBlurViewKey, fadeView,
                                     OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        }
        ...
        if (flag) {  // animate
            CGRect initialFrame = CGRectMake(
                finalFrame.origin.x,
                [UIScreen mainScreen].bounds.size.height +
                    viewControllerToPresent.view.frame.size.height / 2,
    ...
    if (flag) {  // animate
        ...
        [UIView animateWithDuration:ANIMATION_TIME
            ...
            animations:^{
                ...
                // uncomment the line below to have slight rotation during the
                // dismissal
                // self.popupViewController.view.transform =
...
- (CGRect)getPopupFrameForViewController:(UIViewController *)viewController {
    ...
    return CGRectMake(x + viewController.popupViewOffset.x,
                      y + viewController.popupViewOffset.y, frame.size.width,
                      frame.size.height);
}
...
- (void)screenOrientationChanged {
    ...
    [UIView animateWithDuration:ANIMATION_TIME
                     animations:^{
                         self.popupViewController.view.frame =
                             [self getPopupFrameForViewController:
                                       self.popupViewController];
                         if (UIDeviceOrientationIsPortrait(
                         ...
                         if (self.useBlurForPopup) {
                             [UIView animateWithDuration:1.0f
                                 ...
                                 completion:^(BOOL finished) {
                                     [blurView removeFromSuperview];
                                     // popup view alpha to 0 so its not in the
                                     // blur image
...
- (void)setPopupViewController:(UIViewController *)popupViewController {
    objc_setAssociatedObject(self, &CWPopupKey, popupViewController,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)setUseBlurForPopup:(BOOL)useBlurForPopup {
    if ([[[UIDevice currentDevice] systemVersion] floatValue] < 7.0 &&
        ...
        objc_setAssociatedObject(self, &CWUseBlurForPopup,
                                 [NSNumber numberWithBool:NO],
                                 OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    } else {
        objc_setAssociatedObject(self, &CWUseBlurForPopup,
                                 [NSNumber numberWithBool:useBlurForPopup],
                                 OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
}
...
- (void)setPopupViewOffset:(CGPoint)popupViewOffset {
    objc_setAssociatedObject(self, &CWPopupViewOffset,
                             [NSValue valueWithCGPoint:popupViewOffset],
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
